...
@router.post("/matches/{product_id}/recalculate-similarity", response_model=Response, summary="重新计算商品的1688匹配图片相似度")
def recalculate_similarity(
    ...
    _check_product_ownership(db, product_id, current_user.id)
    result = alibaba1688_service.recalculate_similarity_for_product(db, product_id)
