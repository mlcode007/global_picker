"""
拼多多拍照购状态机 —— uiautomator2 版本。

与原版 pdd_photo_flow.py 逻辑完全一致，
仅替换 AdbClient 为 U2Client。
"""
...
from .page_detector_u2 import PageDetector, PageInfo, PageType
...
class PddPhotoFlow:
    """拍照购自动化主流程（uiautomator2 版本）。"""

    def __init__(self, ctx: FlowContext):
        self.ctx = ctx
        self.client = U2Client(serial=ctx.serial)
        self.detector = PageDetector(self.client)
        ...
        for step_enum, step_fn in steps:
            ...
            while retries <= self.ctx.max_retries_per_step:
                ...
                try:
                    ...
                    shot = self.client.screenshot(tag=step_enum.value)
                    ...
                    elapsed = int((time.monotonic() - t0) * 1000)
                    shot = self.client.screenshot(tag=f"{step_enum.value}_err")
                    self.ctx.step_logs.append(StepLog(
                        step=step_enum.value,
    ...
    def _precheck(self):
        ...
        self.client.ensure_screen_on()
        self.client.disable_animations()
        ...
        if not self.client.is_app_installed(PDD_PACKAGE):
        ...
        for perm in [
            ...
            self.client.shell(f"pm grant {PDD_PACKAGE} {perm}")
    ...
    def _push_image(self):
        ...
        self.ctx.remote_image_path = self.client.push_image_to_gallery(
    ...
    def _send_image_to_pdd(self):
        ...
        content_id = self.client.get_media_content_id(remote_path)
        if not content_id:
    ...
    def _wait_result(self):
        ...
        for i in range(25):
            ...
            xml = self.client.dump_ui_xml(tag=f"wait_result_{i}", timeout=5)
            if not xml:
            ...
            if blocker:
                bcode, bmsg = blocker
                shot = self.client.screenshot(tag="wait_result_blocker")
                self.ctx.step_logs.append(StepLog(
    ...
    def _collect_result(self):
        ...
        tag = "result_page_0"
        xml_path = self.client.dump_ui_xml(tag=tag, timeout=4)
        shot = self.client.screenshot(tag=tag)
    ...
    def _has_left_camera_page(self) -> bool:
        ...
        if pkg != PDD_PACKAGE:
            return True
        xml = self.client.dump_ui_xml(tag="left_camera_check", timeout=3)
        if xml:
            return True
        return False
    ...
    def _check_and_handle_permission_by_activity(self) -> bool:
        ...
        xml = self.client.dump_ui_xml(tag="perm_dialog", timeout=5)
        if xml:
            allow_btn = self.detector.find_node_by_text(
                xml, ["始终允许", "仅在使用中允许", "允许", "仅此一次"]
            )
            if allow_btn:
                self.client.tap(*allow_btn["center"])
                return True

        w, h = self.ctx.screen_w, self.ctx.screen_h
        self.client.tap(w // 2, h * 70 // 100)
        return True
    ...
    def _wait_for_package(self, package: str, timeout: int = 8):
        for _ in range(timeout * 2):
            time.sleep(0.5)
            if self.client.current_package() == package:
                return
    ...
    def _dismiss_dialog(self, page: PageInfo):
        ...
        if close_btn:
            self.client.tap(*close_btn["center"])
        else:
            self.client.press_back()
        time.sleep(0.5)
    ...
    def _grant_permission(self, page: PageInfo):
        if not page.xml_path:
            xml_path = self.client.dump_ui_xml(tag="permission")
        else:
