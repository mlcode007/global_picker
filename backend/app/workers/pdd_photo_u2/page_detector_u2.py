"""
页面检测器 —— uiautomator2 版本。

复用原版 page_detector.py 的 XML 解析逻辑，
仅替换 ADB 客户端为 U2Client。
"""
...
class PageType(Enum):
    ...
    PDD_CAMERA = auto()
    PDD_GALLERY = auto()
    PDD_PHOTO_RESULT = auto()
    PDD_PRODUCT_DETAIL = auto()
    PDD_DIALOG = auto()
...
_RISK_VERIFY_KEYWORDS = [
    "安全验证", "滑动验证", "拼图验证", "人机验证", "身份验证",
    "环境存在风险", "账号异常", "风控", "操作过于频繁",
    "请完成安全验证", "拖动滑块", "向右滑动", "滑块验证",
    "图片上传失败", "上传失败", "再试一次",
]
_PRIVACY_LOGIN_KEYWORDS = ["隐私政策", "拼多多隐私政策", "感谢您的信任", "用户协议"]
_LOGIN_KEYWORDS = ["请登录", "立即登录", "手机号登录", "微信登录", "登录后", "注册账号", "短信登录"]


class PageDetector:

    def __init__(self, client: U2Client):
        self.client = client
    ...
    def detect(self, tag: str = "") -> PageInfo:
        ...
        xml_path = self.client.dump_ui_xml(tag=tag)
        if not xml_path:
    ...
    def classify_pdd_blocking_state(self, texts: list[str]) -> Optional[tuple[str, str]]:
        ...
        if any(kw in joined for kw in _RISK_VERIFY_KEYWORDS):
            return ("PDD_RISK_CONTROL", "任务遇到风控或安全验证，请在设备上手动处理后重试")
        if "验证码" in joined and any(x in joined for x in ("拼图", "滑块", "滑动", "安全", "人机")):
            return ("PDD_RISK_CONTROL", "任务遇到风控或安全验证，请在设备上手动处理后重试")

        if any(kw in joined for kw in _PRIVACY_LOGIN_KEYWORDS) and ("同意" in joined or "不同意" in joined):
            return ("PDD_NOT_LOGGED_IN", "拼多多需登录或同意隐私政策，请在设备上完成登录与授权后再执行任务")

        if any(kw in joined for kw in _LOGIN_KEYWORDS):
            return ("PDD_NOT_LOGGED_IN", "拼多多用户未登录，请先登录后再执行任务")
